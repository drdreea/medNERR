...
public class HMMLearnerConstraintsAsFeatures_teaching{
	...
	public HashSet<Integer> wordsInTeaching;

	public HashMap<String, Double[][]> featureProbabilityMatrix;

	public HMMLearnerConstraintsAsFeatures_teaching(TaggingDict td,
		...
		wordsInTeaching = new HashSet<Integer>();
	...
	public void train(Data teachingData, 
			Data labeled_data, Data unlabeled_data,TaggingDict td, 
		...
		trainFromLabeledData(teachingData, labeled_data, td,activeConstraints);
	...
	public void trainFromLabeledData(Data teaching_data,
			Data labeled_data,
		...
		// learn basics from teaching data
		for (int i = 0; i < teaching_data.sen_list.size(); i++) {
			if ((i+1) % 100 == 0) {
				System.out.println("teaching "+(i+1) + "-th sequences");
			}
			int[] vec = teaching_data.sen_list.get(i);
			int[] gold_state_seq = teaching_data.tags_list.get(i); 

			HashMap<String, int[]> features = null;
			if(teaching_data.features_list.isEmpty())
				features = new HashMap<String, int[]>();
				else
					features = teaching_data.features_list.get(i);	
	...
	public HMMLearnerConstraintsAsFeatures_teaching makeCopy( HMMLearnerConstraintsAsFeatures_teaching h,
			TaggingDict td,Vector<Constraint> activeConstraints){
		HMMLearnerConstraintsAsFeatures_teaching ans = new HMMLearnerConstraintsAsFeatures_teaching(td,
	...
	public void HardEMByBeamSearch(ArrayList<int[]> list, ArrayList<int[]> meds,
		...
		HMMLearnerConstraintsAsFeatures_teaching init_m = makeCopy(this,td,activeConstraints);
		...
		for (int s = 0; s < steps; s++) {
			...
			HMMLearnerConstraintsAsFeatures_teaching newHmm=new HMMLearnerConstraintsAsFeatures_teaching(td,
					activeConstraints, this.useFeatures);

			Data newData=new Data(list, new_tag_list, meds, nearbyMeds, pos, reasons, 
					sections, features);
			newHmm.trainFromLabeledData(null, newData,td,activeConstraints);
	...
	public int[] predictSingle1(int seq[],TaggingDict td,Vector<Constraint> activeConstraints, 
		...
		for(int i=0;i<numStates;i++){
			State st=new State(0,seq,i, meds, nearbyMeds, pos, reasons, sections, features);
