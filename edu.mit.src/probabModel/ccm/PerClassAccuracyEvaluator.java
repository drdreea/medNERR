...
public class PerClassAccuracyEvaluator extends TransducerEvaluator {
  ...
  public void evaluateInstanceList (TransducerTrainer tt, InstanceList data, String description)
  {
    ...
    for (int i = 0; i < data.size(); i++) {
      ...
      numOtherTokens += trueOutput.size();
    ...
    double[] allA = new double [numLabels];
    ...
    for (int i = 0; i < numLabels; i++) {
      ...
      double trueFalseTokens = (numOtherTokens - numTrueTokens[i]) - (numPredTokens[i]);
      
      double accuracy = ((double) (numCorrectTokens[i] + trueFalseTokens)/
    		  (numTrueTokens[i] + numPredTokens[i] - numCorrectTokens[i]
    				  + trueFalseTokens));
      ...
      logger.info(description +" label " + label + " P " + f.format (precision)
                  ...
                  " Accuracy " + f.format(accuracy));
    }

    logger.info ("Macro-average F1 "+f.format (MatrixOps.mean (allf)));
    logger.info ("Macro-average Accuracy "+f.format (MatrixOps.mean (allA)));
