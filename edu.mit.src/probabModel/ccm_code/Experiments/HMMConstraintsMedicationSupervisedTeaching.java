...
import probabModel.ccm_code.HMMLearningAndInferenceWithConstraints.HMMLearnerConstraintsAsFeatures_teaching;
 ...
 *
 */
public class HMMConstraintsMedicationSupervisedTeaching {
	...
	public HMMConstraintsMedicationSupervisedTeaching(int nrFeatures, 
	...
	public static void main(String[] args){
		...
		try{
			...
			String[] params = {args[0], args[1], args[2], args[3], "0", "0", "0.9"};
			results_file = args[4];
			args=params;

			HMMConstraintsMedicationSupervisedTeaching classifier = 
					new HMMConstraintsMedicationSupervisedTeaching(numberFeatures, results_file,
	...
	public void testBed (String[] argvs) throws Exception{	
		...
		String teaching_file = argvs[3];
		Parameters.problem = Integer.parseInt(argvs[4]);
		Parameters.method = Integer.parseInt(argvs[5]);
		Parameters.sup_model_weight=Double.parseDouble(argvs[6]);
		...
		all_file_list.add(teaching_file);
		...
		Data teaching_data = new Data(teaching_file, td);
		
		evaluatedifferentConstraintSettings(train_data,unlabeled_data, teaching_data, 
				td);
	}

	public void evaluatedifferentConstraintSettings(Data train_data,
			Data unlabeled_data, Data teaching_data,
			TaggingDict td) throws Exception{
		...
		evaluate( train_data, unlabeled_data,teaching_data, td);
	}
	...
	public void evaluate(Data train_data,Data unlabeled_data,
			Data teaching_data, TaggingDict td){
		ArrayList<String> predictions = new ArrayList<String>();

		HMMLearnerConstraintsAsFeatures_teaching hmm = new 
				HMMLearnerConstraintsAsFeatures_teaching(
				td,
				Parameters.constraintsToApply,
				useFeatures);
		// train the HMM
		hmm.train(teaching_data,
				train_data,unlabeled_data,td,Parameters.constraintsToApply);
